...
// FIXME: Currently FW_DIRECT_PORT_CALLS is set by the top-level CMakeLists.txt,
// see the FIXME in that file for details
...
// The compiler can optimize our code better if it doesn't need to keep running
// after an assertion trips.
...
// Enabled port serialization to reduce code size & memory use.
...
#ifdef BUILD_UT
// fprime-baremetal unit tests depend on FW_FILENAME_ASSERT for some checks
...
#ifdef BUILD_UT
// Must have text logging enabled for UTs
...
#ifdef BUILD_UT
// Must have serialization enabled for UTs
...
// CAUTION: Do not enable inlining unless link time optimization is enabled
#if defined(__clang__)
#define FW_SERIALIZE_FORCE_INLINE_LBB __attribute__((always_inline))
#elif defined(__GNUC__)
#define FW_SERIALIZE_FORCE_INLINE_LBB inline
#else
