...
@ Increased to 150 to accommodate PassiveRateGroup PortCycleTime telemetry (30-element U32 array = 124 bytes)

constant FW_COM_BUFFER_MAX_SIZE = 150
...
@ Specifies number of bytes used by the timestamp included in Event/Channel packets
dictionary constant FW_TIME_SERIALIZED_SIZE = 11

@ Specifies the size of the buffer that contains the serialized log arguments
constant FW_LOG_BUFFER_MAX_SIZE = FW_COM_BUFFER_MAX_SIZE - FW_TIME_SERIALIZED_SIZE - sizeof(FwEventIdType) - sizeof(FwPacketDescriptorType)
...
constant FW_LOG_STRING_MAX_SIZE = 49

@ Specifies the size of the buffer that contains the serialized telemetry value
constant FW_TLM_BUFFER_MAX_SIZE = FW_COM_BUFFER_MAX_SIZE - FW_TIME_SERIALIZED_SIZE - sizeof(FwChanIdType) - sizeof(FwPacketDescriptorType)
...
dictionary constant FW_TLM_STRING_MAX_SIZE = 49
...
constant FW_ASSERT_COUNT_MAX = 2
